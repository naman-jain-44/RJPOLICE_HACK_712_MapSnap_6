...
public class TextRecognitionProcessor extends VisionProcessorBase<Text> {
  ...
  private static final String VEHICLE_NUMBER_REGEX = "^[A-Z]{2}[0-9]{1,2}[A-Z]{1,2}[0-9]{1,4}$";
  ...
  public static boolean isValidIndianVehicleNumber(String input) {
      ...
      for (int i = 0; i < text.getTextBlocks().size(); ++i) {
        ...
        for (int j = 0; j < lines.size(); ++j) {
          ...
          for (int k = 0; k < elements.size(); ++k) {
            ...
            if(isValidIndianVehicleNumber(element.getText())){
              Log.v(
                      "Hackathon",

...
//  private List<ImageLabel> accumulatedLabels = new ArrayList<>();
//  private Runnable logRunnable;
  ...
  protected void onSuccess(
   ...
   // logExtrasForTesting(labels);
    ...
    // Accumulate labels for the current 3 seconds
//    accumulatedLabels.addAll(labels);
//    // Remove any existing callbacks to avoid overlapping
//    handler.removeCallbacks(logRunnable);
//
//    // Schedule a new log after 3 seconds
...
//          Log.v("Hackathon", text + " " + confidence + "  ");
//        }
//
//        // Clear the accumulated labels for new frames
...
//    };
//
//    // Schedule the first log after 3 seconds
//    handler.postDelayed(logRunnable, 3000);
    for (ImageLabel label : labels) {
      String text = label.getText();
      float confidence = label.getConfidence();
